...
class SimpleHTTPProxy(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            log.info("%s not found locally. Searching %d remote repositories" % (
                path, len(SimpleHTTPProxy.mirrors)))
            for mirror in SimpleHTTPProxy.mirrors:
                url = "%s%s" % (mirror, path)
                log.debug("Checking: %s" % (url))
            ...
            log.error(
                "Unhandled exception serving %s. Returning 500: %s" % (path, ex))
    ...
    def serve_local_file(self, path):
        ...
        try:
            ...
            log.error("Local file %s exists, but unable to open: %s" %
                      (full_path, ex))
            return False
    ...
    def log_request(self, code='-', size='-'):
        log.info('%s - - [%s] "%s" %s %s' % (self.address_string(),
                                             self.log_date_time_string(), self.requestline, str(code), str(size)))

    def log_error(self, format, *args):
        log.error("%s - - [%s] %s" % (self.address_string(),
